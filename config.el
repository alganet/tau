...
(defvar tau-minor-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "s-S-p") 'counsel-M-x)
    ...
    (define-key map (kbd "s-S-z") 'undo-tree-redo)
    ...
    (define-key map (kbd "s-S-f") 'counsel-projectile-ag)
    ...
    (define-key map (kbd "s-S-s") 'write-file)
    (define-key map (kbd "C-S-n") 'new-frame)
    (define-key map (kbd "s-S-n") 'new-frame)
    (define-key map (kbd "C-S-w") 'delete-frame)
    (define-key map (kbd "s-S-w") 'delete-frame)
    (define-key map (kbd "C-S-t") 'reopen-killed-file)
    (define-key map (kbd "s-S-t") 'reopen-killed-file)
